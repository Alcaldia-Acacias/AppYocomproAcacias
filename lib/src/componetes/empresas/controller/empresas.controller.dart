...
class EmpresasController extends GetxController {
  ...
  @override
  void onClose() {
    if (pageViewController.hasClients || pageViewController.hasClients == null)
  ...
  void registrarVisita(int idEmpresa, int idUsuario) async {
    final response =
        await this.repositorio.registrarVisitaEmpresa(idEmpresa, idUsuario);
    if (response is ErrorResponse) this._error(response.getError);
    if (response is ResponseEmpresa) {
      print('ok');
    }
